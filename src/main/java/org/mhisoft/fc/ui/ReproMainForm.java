...
//				});
	...
	public void doit() {
		if (props.isSuccess()) {
			...
			props.setDestDir(targetDir);
			props.setVerbose(chkShowInfo.isSelected());
			props.setSourceDir(fldRootDir.getText());
			...
			labelStatus.setText("Done. Dir Removed:" + rdpro.getStatistics().getDirCount()
					+ ", Files removed:" + rdpro.getStatistics().getFilesCount());
		}
	}
	...
	public static void main(String[] args) {
		...
		rdProMain.rdpro = new FastCopy(rdProUI);
		rdProMain.props = rdProUI.parseCommandLineArguments(args);

		if (rdProMain.props.getSourceDir() == null)
			rdProMain.props.setSourceDir(defaultRootDir);

		if (FastCopy.debug) {
			rdProUI.println("set root dir=" + rdProMain.props.getSourceDir());
		}

		//display it
		rdProMain.fldRootDir.setText(rdProMain.props.getSourceDir());
